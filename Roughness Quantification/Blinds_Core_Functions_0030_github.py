...
def readstats_readlastframe(tablefilename):
    """Reads the 'tablefilename' .txt file and imports the data from the last frame at each site. Returns the data as a dictionary called 'data'."""
    global readstats
    if readstats==False:
        Distance=[]
        Avg=[]
...
def remove_chosen_outliers(data_dict_of_roughness_parameters, dataname, tablefilename):
    ...
    the input 'sampledataset_number' refers to a particular set of data:
       #-- 1-4 corresponding to samples 1-4 from Fibre 1, first dataset,
       #-- 5-9 for samples in second dataset of Fibre 1, including the third repeat of the recent without coating sample (F1 T3)
       #-- 10-13 for samples from Fibre 2
       #-- and 14-16 for used tips on fibre 2."""
    ...
    # print(dataname)
    chosen_outliers=[]
    if dataname=='Section 1 - A01':
        chosen_outliers=[775, 1100, 1225, 1750, 1875, 4375, 4625, 4875, 4975, 
                         5000, 5050, 5425, 5775, 5875, 6075, 6150, 6500, 6600, 
                         6625, 6700, 6900, 7125, 7150, 7225, 7400, 7450, 7625, 
                         7650, 8350]                                                #Manually Selected bad frames for A1
        # path = txtfilepaths_1[0]
    elif dataname=='Section 1 - A02':
        chosen_outliers=[50, 200, 825, 1000, 1050, 1400, 1525, 1575, 1750, 1775, 
                         1875, 1950, 2050, 2250, 2275, 2400, 2425, 2575, 2850, 
                         2875, 2925, 3050, 3425, 3450, 3475, 3500, 3550, 3600, 
                         3700, 3725, 4025, 4325, 4900, 5300, 6500, 6850, 7525, 
                         7675]                                                      #Manually Selected bad frames for A2
        # path = txtfilepaths_1[1]
    elif dataname=='Section 1 - A03':
        chosen_outliers=[225, 350, 625, 675, 750, 775, 1225, 1725, 1750, 2025,
                         2175, 2200, 2275, 2775, 3075, 3250, 3700, 4425, 4475, 
                         5000, 5025, 5475, 5650, 6100, 6850, 7050, 7125, 7450, 
                         7525, 7550]                                              #Manually Selected bad frames for A3 ##HEAVILY CONTAMINATED/DAMAGED
        # path = txtfilepaths_1[2]
    elif dataname=='Section 1 - A04':  
        chosen_outliers=[400, 450, 1050, 1075, 1350, 1400, 1800, 1850, 2250, 
                         2325, 2375, 2400, 2500, 2925, 2975, 3000, 3025, 3375, 
                         3450, 3525, 3800, 3825, 4525, 4625, 4875, 4925, 4975, 
                         5000, 6075, 6175, 6300]                                    #Manually Selected bad frames for A4
        # path = txtfilepaths_1[3]
    elif dataname=='Section 1 - A05':  
        chosen_outliers=[725, 975, 1325, 1425, 2125, 2675, 3275, 3300, 3325, 
                         3375, 3500, 3525, 4000, 4850, 5075, 6000, 6975, 7250, 
                         7425]                                                      #Manually Selected bad frames for A5
    elif dataname=='Section 1 - A06':  
        chosen_outliers=[1125, 1350, 2050, 4700, 5900, 6400, 6450, 7775]            #Manually Selected bad frames for A6 ##Quite gunky
    elif dataname=='Section 1 - A07':  
        chosen_outliers=[200, 825, 1150, 1250, 1325, 1825, 1850, 2950, 4425, 4650, 
                         4725, 4775, 4800, 4875, 6125, 6400, 7350, 7375, 7400, 
                         7425, 7500, 7900]                                          #Manually Selected bad frames for A7  ##Some tip wear
    elif dataname=='Section 1 - A08':  
        chosen_outliers=[75, 325, 350, 375, 500, 625, 650, 675, 700, 725, 750, 
                         825, 900, 1000, 1125, 1150, 1200, 1225, 1275, 1600, 1625, 
                         1650, 1675, 1700, 1725, 2025, 2275, 2300, 2375, 2525, 
                         2550, 2800, 2900, 3150, 3175, 3450, 3475, 3500, 4125, 
                         4200, 4275, 4850, 4900, 5000, 5125, 5275, 5325, 5350, 
                         5475, 5725, 6150, 6175, 6575, 7400]                        #Manually Selected bad frames for A8 ##Hairy-like sample
    elif dataname=='Section 1 - A09':  
        chosen_outliers=[1275, 2425, 2475, 2500, 2725, 2850, 3000, 3200, 3425, 
                         3725, 3750, 3875, 4350, 4425, 4475, 4675, 4800, 4900, 
                         4925, 5175, 5400, 5425, 5450, 5675, 5975, 6375, 7125, 
                         7150, 7175, 8400, 8475, 8500, 8525, 8800, 8950]      #Manually Selected bad frames for A9 ##Some tip shape wear
    elif dataname=='Section 1 - A10':  
        chosen_outliers=[0, 25, 50, 75, 200, 325, 475, 900, 1075, 1300, 1325, 
                         1375, 1475, 1500, 1600, 1625, 1800, 1825, 2125, 2325, 
                         2350, 2550, 2575, 2725, 2750, 2825, 2900, 2950, 2975,
                         3075, 3125, 3150, 4525, 4625, 4950, 4975, 5000, 5200, 
                         5675, 5950, 6400, 6450, 6775, 7125, 7150, 7200, 7225, 
                         7575, 7750]                                                #Manually Selected bad frames for A10 ##Heavily contaminated, tip shape effects
    
    
    
    elif dataname=='1st Good Dataset - A01':
        chosen_outliers=[775, 1100, 1225, 1750, 1875, 4375, 4625, 4875, 4975, 
                         5000, 5050, 5425, 5775, 5875, 6075, 6150, 6500, 6600, 
                         6625, 6700, 6900, 7125, 7150, 7225, 7400, 7450, 7625, 
                         7650, 8350]                                            #Manually Selected bad frames for A1
        
    elif dataname=='1st Good Dataset - A02':
        chosen_outliers=[50, 200, 825, 1000, 1050, 1400, 1525, 1575, 1750, 1775, 
                         1875, 1950, 2050, 2250, 2275, 2400, 2425, 2575, 2850, 
                         2875, 2925, 3050, 3425, 3450, 3475, 3500, 3550, 3600, 
                         3700, 3725, 4025, 4325, 4900, 5300, 6500, 6850, 7525, 
    ...
    elif dataname=='1st Good Dataset - A03':  
    ...
    elif dataname=='1st Good Dataset - A04':  
        chosen_outliers=[400, 450, 1050, 1075, 1350, 1400, 1800, 1850, 2250, 
                         2325, 2375, 2400, 2500, 2925, 2975, 3000, 3025, 3375, 
                         3450, 3525, 3800, 3825, 4525, 4625, 4875, 4925, 4975, 
                         5000, 6075, 6175, 6300]                                    #Manually Selected bad frames for A4
    
    elif dataname=='1st Good Dataset - A05':  
        chosen_outliers=[725, 975, 1325, 1425, 2125, 2675, 3275, 3300, 3325, 
                         3375, 3500, 3525, 4000, 4850, 5075, 6000, 6975, 7250, 
                         7425]                                    #Manually Selected bad frames for A5
    
    elif dataname=='1st Good Dataset - A06':  
        chosen_outliers=[125, 150, 1750, 1775, 2025, 3775, 4150, 5700, 6100, 
                         6675, 6925]                                    #Manually Selected bad frames for A06. Very gunky. Need to do a new section. 
    
    elif dataname=='1st Good Dataset - A07':  
    ...
    elif dataname=='1st Good Dataset - A08':  
        chosen_outliers=[50, 300, 450, 975, 1075, 1125, 1200, 1300, 1325, 1350, 
                         1375, 1400, 1475, 1625, 1650, 1775, 1800, 1850, 1925, 
                         1950, 2275, 2300, 2700, 2875, 2950, 2975, 3075, 3125, 
                         3175, 3450, 3600, 3625, 3650, 3775, 3850, 4000, 4550, 
                         4650, 4750, 4875, 4975, 5050, 5075, 5100, 5300, 5450, 
                         5625, 5800, 5850, 5875, 5900, 5975, 6000, 6025, 6050, 
                         6500, 6625, 6650]                                    #Manually Selected bad frames for A08. Very bad condition. No hairs.
    
    elif dataname=='1st Good Dataset - A09':  
        chosen_outliers=[3075, 3700, 4100, 4150, 4250, 4400, 4500, 5000, 5025, 
                         5075, 5350, 5450, 5500, 5600]                                    #Manually Selected bad frames for A09. Very good condition.
    
    elif dataname=='1st Good Dataset - A10':  
        chosen_outliers=[50, 250, 375, 500, 1750, 1775, 1950, 2050, 2125, 2650, 
                         ...
                         6025, 6775, 6875]                                    #Manually Selected bad frames for A10. Very bad tip shape issues.
        
        
        
    elif dataname=='2nd Good Dataset - A01' or dataname=='4th Dataset - A01':
        chosen_outliers=[1200, 1400, 1450, 1775, 1800, 2100, 2175, 2450, 2475, 
                         ...
                         7875, 8025, 8150]                                           #Manually Selected bad frames for A1. Quite a few contaminant particles, but good quality images.
        
    elif dataname=='2nd Good Dataset - A02' or dataname=='4th Dataset - A02':
        chosen_outliers=[300, 325, 550, 1000, 1050, 1200, 1525, 1725, 1775, 
                         ...
                         7775, 7800, 7825]                                            #Manually Selected bad frames for A2. A bit contaminated. Multitip at the end.
        
    elif dataname=='2nd Good Dataset - A03' or dataname=='4th Dataset - A03':  
        chosen_outliers=[600, 650, 750, 1075, 1600, 2125, 2275, 2725, 2750, 
                         2825, 3075, 3225, 3275, 3300, 3475, 3550, 4200, 4225, 
                         4250, 4700, 4725, 4750, 4825, 4850, 5150, 5200, 5325, 
                         5425, 5750, 5775, 5825, 7275, 7725]                                    #Manually Selected bad frames for A3. Somewhat contaminated
    
    elif dataname=='2nd Good Dataset - A04' or dataname=='4th Dataset - A04':  
        chosen_outliers=[275, 350, 400, 575, 600, 1050, 1100, 1150, 1775, 1800, 
                         ...
                         7325]                                    #Manually Selected bad frames for A4. Gunky. 
    
    elif dataname=='2nd Good Dataset - A05' or dataname=='4th Dataset - A05':  
        chosen_outliers=[950, 1175, 1600, 2075, 2150, 2450, 2525, 2550, 2575, 
                         ...
                         6950, 7350, 8075, 8200]                                    #Manually Selected bad frames for A5. Very dirty, lots of artefacts, DISCARD this dataset.
    
    elif dataname=='2nd Good Dataset - A06' or dataname=='4th Dataset - A06':  
        chosen_outliers=[125, 150, 1750, 1775, 2025, 3775, 4150, 5700, 6100, 
                         6675, 6925]                                    #Manually Selected bad frames for A06. Very gunky. Need to do a new section. 
    
    elif dataname=='2nd Good Dataset - A07' or dataname=='4th Dataset - A07':  
        chosen_outliers=[200, 525, 750, 1375, 1925, 1975, 2075, 2125, 2150, 2175, 
                         2200, 2225, 2500, 2775, 3650, 3750, 3975, 4125, 4625, 
                         4750, 4825, 5225, 5275, 5525, 5750, 5900, 6425, 7350, 
                         7375, 7750]                                    #Manually Selected bad frames for A07
    
    elif dataname=='2nd Good Dataset - A08':  
        chosen_outliers=[50, 300, 450, 975, 1075, 1125, 1200, 1300, 1325, 1350, 
                         1375, 1400, 1475, 1625, 1650, 1775, 1800, 1850, 1925, 
                         1950, 2275, 2300, 2700, 2875, 2950, 2975, 3075, 3125, 
                         3175, 3450, 3600, 3625, 3650, 3775, 3850, 4000, 4550, 
    ...
    elif dataname=='2nd Good Dataset - A09' or dataname=='4th Dataset - A09':  
        chosen_outliers=[1450, 1475, 2975, 3075, 3125, 3225, 3500, 3550, 3575, 
                         ...
                         7025, 7050, 7075, 7100, 7125, 7150, 7175, 7200]  #Manually Selected bad frames for A09. Damaged section in middle, multitip at end.
    
    # elif dataname=='2nd Good Dataset - A09' or dataname=='4th Dataset - A09':  
    #     chosen_outliers=[3075, 3700, 4100, 4150, 4250, 4400, 4500, 5000, 5025, 
    #                       5075, 5350, 5450, 5500, 5600]                        #Manually Selected bad frames for A09. Very good condition. Copied from Dataset 1
    
    elif dataname=='2nd Good Dataset - A10' or dataname=='4th Dataset - A10':  
        chosen_outliers=[50, 250, 375, 500, 1750, 1775, 1950, 2050, 2125, 2650, 
                         2975, 3575, 3725, 3900, 4025, 4050, 4450, 4525, 4700, 
                         4750, 4975, 5175, 5225, 5250, 5650, 5950, 5975, 6025, 
                         6025, 6775, 6875]                                    #Manually Selected bad frames for A10. Very bad tip shape issues.
        ...
        chosen_outliers=[250, 300, 375, 400, 500, 625, 675, 700, 775, 925, 1000, 
                         ...
                         7825, 7850, 8525, 8550]                                    #Manually Selected bad frames for A01. #Quite contaminated with salts.
        ...
        chosen_outliers=[25, 50, 200, 225, 250, 275, 300, 325, 375, 400, 425, 
                         ...
                         7600, 7625, 7650, 7675, 7700, 7725, 7750, 7775, 7800]      #Manually Selected bad frames for A03. 7325 onwards example of multitip.
        ...
        chosen_outliers=[0, 50, 75, 100, 125, 150, 175, 200, 225, 250, 275, 
                         ...
                         5675, 5700, 5925, 6725, 6775, 6850, 7675, 7725]            #Manually Selected bad frames for A06. Lots of bad tip at the start 
        ...
        chosen_outliers=[50, 1875, 1900, 2450, 2450, 2475, 2500, 2525, 2550,
                         ...
                         6575, 6600, 6625, 6650, 6675, 6700, 6725, 6750, 6775,
                         6800, 6825, 6850, 6875, 6900, 6925, 6950, 6975, 7000, 
                         7025, 7050]                                    #Manually Selected bad frames for A09. Section of multitip and Tip shape change at end.
...
def read_SEM_data(datafile):
    """Reads the 'datafile' .txt file and imports the data for each of the roughness parameters. Returns the data as a dictionary called 'data'.
    v0026 code reads 05Mar data
    v0027 code reads 08Mar data
    v0028 code reads 04Apr data"""
    
    # datafile='L-A_OLD_Roughness_Results_26FEB_Dhilans_Edit_of_names.txt'
    ...
    OLC_thickness_means=[]
    OLC_thickness_std_devs=[]
    OLC_thickness_std_unc=[]
    
    SiC_slope_means=[]
    SiC_slope_std_devs=[]
    SiC_slope_st_unc=[]
    SiC_OLC_slope_means=[]
    SiC_OLC_slope_std_devs=[]
    SiC_OLC_slope_st_unc=[]
    OLC_ILC_slope_means=[]
    OLC_ILC_slope_std_devs=[]
    OLC_ILC_slope_st_unc=[]
    
    
    with open(datafile, 'r') as csvfile:
        plots= csv.reader(csvfile, delimiter='\t')
        for line_count, row in enumerate(plots):
            if line_count==0:
                pass          #skips header line
            else: 
                Number.append(int(row[1]))
                
                # SiC_Ra_means.append(float(row[2]))
                # SiC_Ra_std_devs.append(float(row[3]))
                # SiC_Ra_std_unc.append((float(row[3])/math.sqrt(float(row[1]))))
                # SiC_Rq_means.append(float(row[4]))
                # SiC_Rq_std_devs.append(float(row[5]))
                # SiC_Rq_std_unc.append((float(row[5])/math.sqrt(float(row[1]))))
                # SiC_Rz_means.append(float(row[6]))
                # SiC_Rz_std_devs.append(float(row[7]))
                # SiC_Rz_std_unc.append((float(row[7])/math.sqrt(float(row[1]))))
                # SiC_slope_means.append(float(row[8]))
                # SiC_slope_std_devs.append(float(row[9]))
                # SiC_slope_st_unc.append((float(row[9])/math.sqrt(float(row[1]))))
                
                SiC_OLC_Ra_means.append(float(row[2]))
                SiC_OLC_Ra_std_devs.append(float(row[3]))
                SiC_OLC_Ra_std_unc.append((float(row[3])/math.sqrt(float(row[1]))))
                SiC_OLC_Rq_means.append(float(row[4]))
                SiC_OLC_Rq_std_devs.append(float(row[5]))
                SiC_OLC_Rq_std_unc.append((float(row[5])/math.sqrt(float(row[1]))))                
                SiC_OLC_Rz_means.append(float(row[6]))
                SiC_OLC_Rz_std_devs.append(float(row[7]))
                SiC_OLC_Rz_std_unc.append((float(row[7])/math.sqrt(float(row[1]))))
                SiC_OLC_slope_means.append(float(row[8]))
                SiC_OLC_slope_std_devs.append(float(row[9]))
                SiC_OLC_slope_st_unc.append((float(row[9])/math.sqrt(float(row[1]))))
                
                
                OLC_ILC_Ra_means.append(float(row[10]))
                OLC_ILC_Ra_std_devs.append(float(row[11]))
                OLC_ILC_Ra_std_unc.append((float(row[11])/math.sqrt(float(row[1]))))
                OLC_ILC_Rq_means.append(float(row[12]))
                OLC_ILC_Rq_std_devs.append(float(row[13]))
                OLC_ILC_Rq_std_unc.append((float(row[13])/math.sqrt(float(row[1]))))
                OLC_ILC_Rz_means.append(float(row[14]))
                OLC_ILC_Rz_std_devs.append(float(row[15]))
                OLC_ILC_Rz_std_unc.append((float(row[15])/math.sqrt(float(row[1]))))
                OLC_ILC_slope_means.append(float(row[16]))
                OLC_ILC_slope_std_devs.append(float(row[17]))
                OLC_ILC_slope_st_unc.append((float(row[17])/math.sqrt(float(row[1]))))
                
                OLC_thickness_means.append(float(row[18]))
                OLC_thickness_std_devs.append(float(row[19]))
                OLC_thickness_std_unc.append((float(row[19])/math.sqrt(float(row[1]))))
                
                
    # N={'N': Number}
    
    # Ra={'Means': SiC_Ra_means, 'Standard Deviations': SiC_Ra_std_devs, 'Standard Uncertainty': SiC_Ra_std_unc}
    # Rq={'Means': SiC_Rq_means, 'Standard Deviations': SiC_Rq_std_devs, 'Standard Uncertainty': SiC_Rq_std_unc}
    # Rz={'Means': SiC_Rz_means, 'Standard Deviations': SiC_Rz_std_devs, 'Standard Uncertainty': SiC_Rz_std_unc} 
    # Slope={'Means': SiC_slope_means, 'Standard Deviations': SiC_slope_std_devs, 'Standard Uncertainty': SiC_slope_st_unc}
    # SiC = {'Ra': Ra, 'Rq': Rq, 'Rz': Rz, 'Rdq': Slope}

    Ra={'Means': SiC_OLC_Ra_means, 'Standard Deviations': SiC_OLC_Ra_std_devs, 'Standard Uncertainty': SiC_OLC_Ra_std_unc}
    Rq={'Means': SiC_OLC_Rq_means, 'Standard Deviations': SiC_OLC_Rq_std_devs, 'Standard Uncertainty': SiC_OLC_Rq_std_unc}
    Rz={'Means': SiC_OLC_Rz_means, 'Standard Deviations': SiC_OLC_Rz_std_devs, 'Standard Uncertainty': SiC_OLC_Rz_std_unc} 
    Slope={'Means': SiC_OLC_slope_means, 'Standard Deviations': SiC_OLC_slope_std_devs, 'Standard Uncertainty': SiC_OLC_slope_st_unc}
    SiC_OLC = {'Ra': Ra, 'Rq': Rq, 'Rz': Rz, 'Rdq': Slope}
    
    Ra={'Means': OLC_ILC_Ra_means, 'Standard Deviations': OLC_ILC_Ra_std_devs, 'Standard Uncertainty': OLC_ILC_Ra_std_unc}
    Rq={'Means': OLC_ILC_Rq_means, 'Standard Deviations': OLC_ILC_Rq_std_devs, 'Standard Uncertainty': OLC_ILC_Rq_std_unc}
    Rz={'Means': OLC_ILC_Rz_means, 'Standard Deviations': OLC_ILC_Rz_std_devs, 'Standard Uncertainty': OLC_ILC_Rz_std_unc} 
    Slope={'Means': OLC_ILC_slope_means, 'Standard Deviations': OLC_ILC_slope_std_devs, 'Standard Uncertainty': OLC_ILC_slope_st_unc}
    OLC_ILC = {'Ra': Ra, 'Rq': Rq, 'Rz': Rz, 'Rdq': Slope}

    OLC_Thickness={'Means': OLC_thickness_means, 'Standard Deviations': OLC_thickness_std_devs, 'Standard Uncertainty': OLC_thickness_std_unc}
    ...
    data = {
        ...
        'SiC-OLC'       :   SiC_OLC,
        'OLC-ILC'       :   OLC_ILC,
        'OLC Thickness' :   OLC_Thickness            
        }
    return data
    
def read_all_SEM_data():
    SEM_dataset_names=['L-A', 'C-A', 'L-B']
    
    # datafiles=['L-A_Slope_Roughness_Results_01Mar.txt',
    #             'C-A_Slope_Roughness_Results_01Mar.txt',
    #             'L-B_Slope_Roughness_Results_01Mar.txt']
    
    # datafiles=['L-A_New_05Mar_Interfacial_Roughness_Results.txt',
    #            'C-A_New_05Mar_Interfacial_Roughness_Results.txt',
    #            'L-B_New_05Mar_Interfacial_Roughness_Results.txt']
    
    # datafiles=['L-A_New_08Mar_Interfacial_Roughness_Results.txt',
    #             'C-A_New_08Mar_Interfacial_Roughness_Results.txt']
    
    datafiles=['L-A_04Apr_Interfacial_Roughness_Results.txt',
               'C-A_04Apr_Interfacial_Roughness_Results.txt']
    
    all_data={}
    os.chdir('F:/3.SiC Blinds/')
    for a, filename in enumerate(datafiles):  #Iterating through the datafiles for each dataset
        data = read_SEM_data(filename)
        append_value(all_data, SEM_dataset_names[a], data)
    return all_data
...
def mean_and_uncertainty_plot(roughness_data, column_number, method_number, sample_names, fibre_section_title, tagline, savefile=False, only_errors=False):
    ...
    if n_sites<30:
        #Uses the t-score if the number of measurements is too small (e.g. bend strain data)
        t_score_95=[]
        t_score_99_7=[]
